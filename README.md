<div align="center">

## 👋 Yahia Elboukili  
**Systems Engineering · Linux · DevOps**

C • C++ • Python • Bash • Ansible • Docker • AWS

[🌐 Portfolio](https://www.yahia-elboukili.com) ·
[LinkedIn](https://www.linkedin.com/in/yahia-elboukili/) ·
[LeetCode](https://leetcode.com/u/yahyaeb/) ·
[CodinGame](https://www.codingame.com/profile/826288cc5efb8c41452c93e449b6de350409346) ·
[Email](mailto:yahya.elboukili1@gmail.com)

</div>

---

## About Me

- Systems & DevOps engineering student at **École 42 Nice**
- Hands-on experience in Linux administration, infrastructure automation, and cloud deployment
- Production experience building Ansible compliance pipelines for multi-server fleet management
- CompTIA Network+, Security+

---

## 🧰 Tech Stack

**Languages & Systems:**  
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat-square&logo=go&logoColor=white)

**Backend & APIs:**  
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat-square&logo=css3&logoColor=white)

**Development Environment:**  
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![macOS](https://img.shields.io/badge/macOS-000000?style=flat-square&logo=apple&logoColor=white)

**Cloud & Infrastructure:**  
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat-square&logo=ansible&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white)

---

## Featured Projects

- **Inception** — Dockerized multi-service stack (Nginx, WordPress, MariaDB, TLS)
- **ft_ping** — Low-level C reimplementation of ping with raw ICMP sockets
- **Minishell** — Custom Unix shell in C (pipes, redirections, env, process control)
- **ft_transcendence** — Full-stack app with real-time multiplayer game
- **CI/CD Pipeline** — GitHub Actions for build/test/deploy
- **AWS Secure Static Website** — S3 + CloudFront + Route 53 + ACM

👉 [View all projects](https://github.com/yahyaeb/projects-portfolio)

---

## Currently Working On

- 🦫 Taskmaster — job control daemon in Go (learning Go for cloud-native tooling)

---

## Outside Tech

BJJ • endurance sports • hackathons • problem-solving under pressure

- **Hackathons:**  
  - *Code4Sud* – disaster-preparedness innovation challenge  
  - *Tech4Tourism* – improving travel and tourism through digital tools

*When I'm not coding, you'll find me on the mat, the trail, or in the gym!*

*Feel free to reach out for collaborations, opportunities, or just a friendly chat!*
